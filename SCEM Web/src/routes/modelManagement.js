...
const ModelManagement = () => {
    ...
    <div className="search-input-container">
        <label htmlFor={"modelManagement-searchInput"}>Model</label>
        <input
    ...
    <div className="search-input-container">
        <label htmlFor={"modelManagement-manufactureSelect"}>Manufacture</label>
        <select
    ...
    <button id={"modelManagement-searchButton"} type={"submit"}>
        Search
            ...
            <div id={"modelManagement-createButtonDiv"}>
                <button id={"modelManagement-createButton"} onClick={() => window.location.href = "/modelManagement/create"}>Create New
    ...
    {searchResults.slice((currentPageNumber - 1) * MAX_RESULTS, currentPageNumber * MAX_RESULTS).map((doc, index) =>
        <tr key={doc.id}>
            ...
            <td
                ...
                onBlur={e => {
                    const editedModelCopy = { ...editedModels };
                    editedModelCopy[index] = { ...editedModelCopy[index], modDescription: e.target.innerText };
                    setEditedModels(editedModelCopy);
                }}
            >
            ...
            <td
                ...
                onBlur={e => {
                    const editedModelCopy = { ...editedModels };
                    editedModelCopy[index] = { ...editedModelCopy[index], modEquipmentType: e.target.innerText };
                    setEditedModels(editedModelCopy);
                }}
            >
            ...
            <td className={"modelManagement-tableElement"}>
                <div className="modelManagement-tableButtonContainer">
                    {!editing[index] ?
                        <button onClick={() => handleEdit(index)} className={"modelManagement-editButton"}>Edit</button>
                        : <button onClick={() => handleSave(index, doc.id)} className={"modelManagement-saveButton"}>Save</button>}
                </div>
            </td>
        </tr>
    )}
